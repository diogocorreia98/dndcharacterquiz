...
const buildLocalizedText = (builder) => {
  const translations = {};
  SUPPORTED_LANGUAGES.forEach((lang) => {
    const text = builder(lang);
    if (text) {
...
const formatValueListFromMap = (metadata, mapKey, values, lang) => {
  if (!Array.isArray(values) || !values.length) {
    return '';
  }
  const valueMap = metadata?.value_map?.[mapKey];
  return values
    .map((value) => {
      const entry = valueMap?.[value];
      if (entry && typeof entry === 'object') {
        return entry[lang] ?? entry[DEFAULT_LANGUAGE] ?? String(value);
      }
      return String(value);
    })
    .filter(Boolean)
...
const ABILITY_LABELS = {
  STR: withTranslations('Força (STR)', 'Strength (STR)'),
  DEX: withTranslations('Destreza (DEX)', 'Dexterity (DEX)'),
  CON: withTranslations('Constituição (CON)', 'Constitution (CON)'),
  INT: withTranslations('Inteligência (INT)', 'Intelligence (INT)'),
  WIS: withTranslations('Sabedoria (WIS)', 'Wisdom (WIS)'),
  CHA: withTranslations('Carisma (CHA)', 'Charisma (CHA)'),
};

const SPECIES_DESCRIPTION_LABELS = {
  parent: withTranslations('Espécie base', 'Parent species'),
  commonality: withTranslations('Presença', 'Commonness'),
  height: withTranslations('Altura', 'Height'),
  traits: withTranslations('Traços', 'Traits'),
  ancestry: withTranslations('Ancestralidade', 'Ancestry'),
  power: withTranslations('Afinidade de poder', 'Power focus'),
};

const DRAGONBORN_ANCESTRY_LABELS = {
  chromatic: withTranslations('Dragão cromático', 'Chromatic dragon'),
  metallic: withTranslations('Dragão metálico', 'Metallic dragon'),
  gem: withTranslations('Dragão de gema', 'Gem dragon'),
};

const GOLIATH_POWER_LABELS = {
  offensive: withTranslations('Proeza ofensiva', 'Offensive prowess'),
  defensive: withTranslations('Força defensiva', 'Defensive resilience'),
  mobility: withTranslations('Mobilidade ágil', 'Mobile agility'),
};
...
const SUBCLASS_DESCRIPTION_LABELS = {
  class: withTranslations('Classe', 'Class'),
  group: withTranslations('Tipo', 'Type'),
  variants: withTranslations('Variações', 'Variants'),
  abilities: withTranslations('Atributos', 'Abilities'),
  armor: withTranslations('Armadura', 'Armor'),
...
const FIGHTING_STYLE_OPTION_MAP = {
  ...
  FIGHTER_FIGHTING_STYLE_UNARMED: 'UNARMED',
  RANGER_FIGHTING_STYLE_CLASSIC: 'CLASSIC',
  RANGER_FIGHTING_STYLE_DRUIDIC: 'DRUIDIC_WARRIOR',
  RANGER_FIGHTING_STYLE_THROWN: 'THROWN',
  RANGER_FIGHTING_STYLE_UNARMED: 'UNARMED',
  PALADIN_FIGHTING_STYLE_CLASSIC: 'CLASSIC',
  PALADIN_FIGHTING_STYLE_BLESSED: 'BLESSED_WARRIOR',
  PALADIN_FIGHTING_STYLE_THROWN: 'THROWN',
...
const buildSpeciesDescription = function (option) {
  ...
  return buildLocalizedText((lang) => {
    const parts = [];
    const commonality = formatValueListFromMap(metadata, 'species_commonality', entry.species_commonality, lang);
    if (commonality) {
      parts.push(`${SPECIES_DESCRIPTION_LABELS.commonality[lang]}: ${commonality}`);
    }
    const height = formatValueListFromMap(metadata, 'height', entry.height, lang);
    if (height) {
      parts.push(`${SPECIES_DESCRIPTION_LABELS.height[lang]}: ${height}`);
    }
    const traits = formatValueListFromMap(metadata, 'animal_likeness', entry.animal_likeness, lang);
    if (traits) {
      parts.push(`${SPECIES_DESCRIPTION_LABELS.traits[lang]}: ${traits}`);
    }
    return parts.join(' · ');
  });
};

const buildSubspeciesDescription = function (option) {
  const entry = option?.datasetEntry;
  if (!entry) {
    return null;
  }
  const metadata = this.quizData.metadata ?? {};
  const parentSpecies = this.getDatasetEntry('species', entry.species_code);
  return buildLocalizedText((lang) => {
    const parts = [];
    if (parentSpecies) {
      const parentName = parentSpecies[lang] ?? parentSpecies[DEFAULT_LANGUAGE];
      if (parentName) {
        parts.push(`${SPECIES_DESCRIPTION_LABELS.parent[lang]}: ${parentName}`);
      }
    }
    const commonality = formatValueListFromMap(metadata, 'species_commonality', entry.species_commonality, lang);
    if (commonality) {
    ...
    if (Array.isArray(entry.dragonborn_ancestry_type) && entry.dragonborn_ancestry_type.length) {
      const ancestry = entry.dragonborn_ancestry_type
        .map((type) => DRAGONBORN_ANCESTRY_LABELS[type]?.[lang] ?? DRAGONBORN_ANCESTRY_LABELS[type]?.[DEFAULT_LANGUAGE] ?? type)
        .join(', ');
      parts.push(`${SPECIES_DESCRIPTION_LABELS.ancestry[lang]}: ${ancestry}`);
    }
    if (Array.isArray(entry.goliath_power_type) && entry.goliath_power_type.length) {
      const power = entry.goliath_power_type
        .map((type) => GOLIATH_POWER_LABELS[type]?.[lang] ?? GOLIATH_POWER_LABELS[type]?.[DEFAULT_LANGUAGE] ?? type)
        .join(', ');
      parts.push(`${SPECIES_DESCRIPTION_LABELS.power[lang]}: ${power}`);
    }
    return parts.join(' · ');
  });
};

const buildSubsubspeciesDescription = function (option) {
  const entry = option?.datasetEntry;
  if (!entry) {
    return null;
  }
  const metadata = this.quizData.metadata ?? {};
  const parentSubspecies = this.getDatasetEntry('subspecies', entry.subspecies_code);
  return buildLocalizedText((lang) => {
    const parts = [];
    if (parentSubspecies) {
      const parentName = parentSubspecies[lang] ?? parentSubspecies[DEFAULT_LANGUAGE];
      if (parentName) {
        parts.push(`${SPECIES_DESCRIPTION_LABELS.parent[lang]}: ${parentName}`);
      }
    }
...
const buildFightingStyleDescription = function (option) {
  const styleCode = FIGHTING_STYLE_OPTION_MAP[option?.value];
  if (!styleCode) {
    return null;
  }
...
const AUTO_DESCRIPTION_BUILDERS = {
  ...
  q_class_adjustment_paladin: buildFightingStyleDescription,
};
...
class QuizApp {
  ...
  getQuestionText(nodeId, node) {
    ...
    const localeKey = this.getLocaleKey();
    const translationEntry = this.quizData.descriptionGroups?.question_texts?.[nodeId];
  ...
  resolveOptionDescriptionEntry(nodeId, option) {
    if (!option) {
      return null;
    }
    if (option.description_translations) {
      return option.description_translations;
    }
    if (option.description && typeof option.description === 'object') {
    ...
    const optionMapEntry = this.quizData.optionLabels?.[nodeId]?.[option.value];
    if (optionMapEntry?.description) {
      return optionMapEntry.description;
    }
    ...
    if (typeof builder === 'function') {
      return builder.call(this, option, nodeId);
    }
    return null;
  }

  getOptionDescription(nodeId, option) {
    const localeKey = this.getLocaleKey();
    const entry = this.resolveOptionDescriptionEntry(nodeId, option);
    if (!entry) {
      return '';
    }
  ...
  getDatasetEntry(datasetName, code) {
    if (!datasetName || code === undefined || code === null) {
      return null;
    }
    if (!this.datasetIndexes) {
      this.datasetIndexes = {};
    }
    const cacheKey = datasetName;
    if (!this.datasetIndexes[cacheKey]) {
      const entries = this.quizData.metadata?.datasets?.[datasetName];
      const map = new Map();
      if (Array.isArray(entries)) {
        entries.forEach((entry) => {
          const entryCode = entry?.code ?? entry?.id ?? entry?.value;
          if (entryCode !== undefined && entryCode !== null) {
            map.set(String(entryCode).toUpperCase(), entry);
          }
        });
      }
      this.datasetIndexes[cacheKey] = map;
    }
    const lookupKey = String(code).toUpperCase();
    return this.datasetIndexes[cacheKey].get(lookupKey) ?? null;
  }
    ...
    node.options.forEach((option) => {
      ...
      if (translation && typeof translation === 'object') {
        ...
        if (description && typeof description === 'object') {
          option.description_translations = { ...description };
        }
      }
    });
  });
...
const loadQuizData = async (manifestUrl) => {
  ...
  const mergeDescriptionGroups = (current, incoming) => {
  ...
  for (const descriptionFile of descriptionFiles) {
    ...
    if (descriptions && typeof descriptions === 'object') {
      descriptionGroups = mergeDescriptionGroups(descriptionGroups, descriptions);
    }
  }
